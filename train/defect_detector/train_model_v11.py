...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Обучение YOLO11 для второй модели"
    )
    parser.add_argument(
        "--data",
        type=str,
    ...
    parser.add_argument("--name", type=str, default="yolo11n_run", help="Имя запуска")
    return parser.parse_args()


def main():
    args = parse_args()
    project_dir = os.path.abspath("runs/detect_yolo11n")

    print("=" * 60)
    print("🚀 ЗАПУСК ОБУЧЕНИЯ YOLO11n")
    print("=" * 60)
